...
clear; clc; close all;
doWrite = true; % set 'true' to have individual xls files for each
...
% Make the output directory and xls file 
[pathstr,name,ext] = fileparts(d);
dateTag = datestr(now);
dateTag = strrep(dateTag,':','.');
writename = [name ' results ' dateTag '.xls'];
usrinput = inputdlg({'Please set the output file name'},'output name',1,{writename});
writename = [d filesep usrinput{1}];
% Create the outuput file and write the parameters
pOut = {'Crop Radius', crop_r; 'SIFT Threshold', im_thresh; 'Gaussian Template Size', TemplateSize; ...
    'Gaussian SD', SD; 'Gaussian Threshold', gaussianTh; 'Size filter minimum, nm' , minSize; 'Size filter maximum, nm', maxSize; 'Particle material', pMaterial; 'Number of Spots', length(preFList)};
xlswrite(writename, pOut, 1, 'A1');

headingLine = {'Chip Name', 'Spot Name', 'Pre TS', 'Post TS', 'Unmatched Pre','Size fltd Unmatched Pre', 'Matches', 'Size filt Matches', ...
    'Unmatched Post', 'Size filt Unmatched Post', 'Area, mm^2', 'Density, particles/mm^2', '', 'Verifification Image Number'};
xlswrite(writename, headingLine, 1, 'E5');

chipNameColumn = cellfun(@regexpi, matchedSpotFList(:,1), ChipNamesPatt,tokk);
...
TSColumns = cellfun(@regexpi,matchedSpotFList,{tsPatt{:}; tsPatt{:}}',{tokk{:}; tokk{:}}'); % Write the timestamp columns
TSColumns = reshape([TSColumns{:}],length(postFList),2);

xlsOut = [chipNameColumn, spotNameColumn, TSColumns];
xlswrite(writename,xlsOut, 1, 'E6');
...
for i = 1:length(postFList)
    ...
    [matchedContrastPost, unmatchedContrastPost, unmatchedContrastPre, theta, crop_r_out] = pcSIFT(preIm, postIm, ...
    ...
    if isempty(matchedContrastPost)
        ...
        spotOut = {'Couldn''t find a good alignment this spot'};
        xlswrite(writename, spotOut, 1, ['P' num2str(i+5)]);
        disp(['Alignment failed for chip ' chipNameColumn{i} ', spot ' spotNameColumn{i}]);
    else
        % write to xls sheet
        unmatchedPreSizes = contrastToSize(unmatchedContrastPre,pMaterial);
        filteredPre = sum(unmatchedPreSizes(:)> minSize & unmatchedPreSizes(:)< maxSize);
        
        matchSizes = contrastToSize(matchedContrastPost,pMaterial);
        filteredMatches = sum(matchSizes(:)> minSize & matchSizes(:)< maxSize);
        
        unmatchedPostSizes = contrastToSize(unmatchedContrastPost,pMaterial);
        filteredPost = sum(unmatchedPostSizes(:)> minSize & unmatchedPostSizes(:)< maxSize);
        
        A = pi*crop_r_out^2 * pixelAreaMicrons/1000;
        densityFilteredPost = filteredPost/A;
        spotOut = {length(unmatchedPreSizes),filteredPre, length(matchSizes),filteredMatches, length(unmatchedPostSizes), filteredPost, A, densityFilteredPost, '', imageCounter};
        xlswrite(writename, spotOut, 1, ['I' num2str(i+5)]);
        imageCounter = imageCounter+1;
        
        % write individual xls if desired
        if doWrite
            [~,thisname] = fileparts(matchedSpotFList{i,2});
            save([d filesep 'histogram chip' chipNameColumn{i} ' spot' spotNameColumn{i} '.mat'], 'unmatchedContrastPost','matchedContrastPost');
        end
    end
